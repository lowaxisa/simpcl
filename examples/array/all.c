...
void populate_array(scl_array_t *a, size_t n) {
    for (int i = n - 1; i >= 0; i--) {
        scl_array_push(a, &i);
    }
}
...
int main() {
    ...
    int num = 9;
    printf("\nelement 9 is in mem addr: %p\n", scl_array_find(a, &num, find_cb));
    scl_array_append(copy, a, NULL);
    printf("numbers in array copy\n");
    scl_array_print(copy, cb);
    scl_array_append(copy, copy, NULL);
    printf("numbers in array copy\n");
    scl_array_print(copy, cb);
